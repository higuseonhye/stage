...
export function AgentCard({ agent, streaming, children, footer }: Props) {
  ...
  return (
    <Card
      className={cn(
        "flex min-h-[220px] flex-col border-2 bg-card/40 p-3 shadow-none backdrop-blur-sm",
        c.border,
        streaming && "ring-2",
        streaming && c.ring,
      )}
    >
      ...
      <div className="text-muted-foreground min-h-0 flex-1 font-mono text-xs leading-relaxed whitespace-pre-wrap">
        {children}
      </div>
